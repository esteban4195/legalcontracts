...
@Component({
  ...
  templateUrl: 'contract-new.page.html',
  styleUrls: ['contract-new.page.scss'],
  standalone: true,
  imports: [CommonModule, FormsModule],
})
export class ContractNewPage implements OnInit {
  // Form fields
  title = '';
  content = '';
  ...
  ngOnInit(): void {
    if (this.auth.getCurrentUser()?.system_role !== 'ADMIN') {
  ...
  roleBadgeClass(role: string): string {
    return { CLIENTE: 'badge-cliente', PROVEEDOR: 'badge-proveedor', TESTIGO: 'badge-testigo' }[role] ?? '';
  }

  roleLabel(role: string): string {
    return { CLIENTE: 'Cliente', PROVEEDOR: 'Proveedor', TESTIGO: 'Testigo' }[role] ?? role;
  }
  ...
  save(): void {
    ...
    this.contractsSvc.create(payload).subscribe({
      next: () => {
        this.saving = false;
        this.router.navigate(['/contracts']);
      },
      error: (err) => {
        this.saving = false;
        this.errorMsg = err.error?.detail || 'Error al crear el contrato';
      },
    });
  }
